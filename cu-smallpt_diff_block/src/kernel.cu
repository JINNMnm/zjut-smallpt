...
namespace smallpt {
	...
	static void Render(std::uint32_t nb_samples, dim3 nthreads, dim3 nblocks, FILE* log_fp) {
		...
		std::printf("[Block=%ux%u] Samples=%u | Kernel: %.2f ms | Total: %.2f ms | Eff: %.3f\n",
					nthreads.x, nthreads.y, nb_samples, kernel_time_ms, total_time_ms, efficiency);
...
int main() {
    ...
    std::fprintf(log_fp, "block_x,block_y,samples,kernel_time_ms,total_time_ms,efficiency\n");

    for (int block_size = 16; block_size <= 64; block_size *= 2) {
        dim3 nthreads(block_size, block_size);
        dim3 nblocks((w + block_size - 1) / block_size, (h + block_size - 1) / block_size);

        for (int samples = 40; samples <= 2200; samples += 160) {
            smallpt::Render(samples, nthreads, nblocks, log_fp);
        }
    }
