...
namespace smallpt {
	...
	static void Render(std::uint32_t nb_samples, std::uint32_t frame_idx) noexcept {
		...
		char filename[64];
		std::snprintf(filename, sizeof(filename), "frames/frame_%04u.ppm", nb_samples);
...
int main(int argc, char* argv[]) {
    for (std::uint32_t samples = 0, frame = 0; samples <= 100; samples += 1, ++frame) {
        std::printf("Rendering frame %u with %u samples...\n", frame, samples);
        smallpt::Render(samples, frame);
    }
    return 0;
}